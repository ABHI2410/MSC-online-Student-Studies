...
class LoginManager{
    ...
    static getLoginManager(){
      ...
      authfetch(url,options={}){
        ...
        if (val){
          newOptions.headers = {
            "Authorization": "Bearer "+localStorage.getItem("LoginManager.accessToken"),...options.headers}//enter token here
      ...
      async post(url,requestBody){
        ...
        try{
          ...
          if (requestBody instanceof FormData){
            res = await this.getJson(this.authfetch(url,{method:"POST",body:requestBody}));
          }else{
            res = await this.getJson(this.authfetch(url,{method:"POST",body:JSON.stringify(requestBody)}));
          }
        } catch (error){
